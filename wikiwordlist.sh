...
if wget http://wikimedia.bytemark.co.uk/enwiki/$today/enwiki-$today-pages-articles.xml.bz2 > /dev/null 2>&1; then
		echo  "downloaded enwiki-$today-pages-articles.xml.bz2"
	else
		echo "there is no file for $today. append the date to the script to download an alternative"
	exit 1
fi
